...
der die Zusammensetzung des Algoritmusses dann zunächst erklärt und denn entsprechenden Quelltext dann auch noch ausführlich vorstellt.
...
\begin{figure}
    ...
    \caption{Zur Eckenberechnung genutzte Vektoren}
...
% $$ C_d = cP + btF * (width - dP) $$
    ...
    \begin{align}
        ...
        \begin{split}\label{eq:4}
            C_c ={}& C_d + sDD * length
        \end{split}\\
            C_d ={}& cP + btF * (width - dP)\label{eq:5}
        \end{align}
...
Zunächst werden die beiden hinteren, äußeren Punkte des Korridors bestimmt. Da die Ecken eines Raumes immer im Uhrzeigersinn gespeichert werden lassen sich die Ecke des Raumes, die $S$ entspricht und die im Uhrzeigersinn darauf folgende auswählen. %TODO: formulierung
...
In dem hier vorgestelten Projekt werden die Wände auf zwei verschiedene Arten generiert. Die erste Art, fortlaufend \textquote{realistische} Variante genannt, generiert die Wände zwischen den Räumen so, wie sie auch in echt existieren könnten.
Dazu gibt es einen ausgefeilten Algortithmus, der alle Eventualitäten der Raumgenerierung kennt und die Wände so zwischen den Räumen platziert, dass die Räume noch möglichst viel Platz bieten, aber auch runherum von Wänden umgeben sind. Der Code für diese Generierung wird in der \textquote{RoomGenerator}-Klasse in den beiden \textquote{GenerateWallsLegacy}-Methoden ausgeführt. Diese Variante wird in den beiden Versuchsbedingungen \textquote{Teleport} und \textquote{Joystick} verwendet, weil diese keine Impossible Spaces beinhalten.
...
In der Abbildung x %TODO: figure:

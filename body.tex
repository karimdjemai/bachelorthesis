...
%TODO: Raumverständnis ausarbeiten
...
\chapter{Konzipierung der Pilotierungsstudie}\label{chapter:experiment}
    ...
    \section{Hypothesen und Messvariablen}
        ...
        \begin{enumerate}
            \item Die \textquote{Real-Walking} Fortbewegungsart ermöglicht besseres Raumverständnis %(spatial knowledge)
            , als die beiden alternativen \textquote{Joystick} und \textquote{Teleportaion}. (Basiert auf:) %TODO: langebehn 2018 und quellen daraus: 7 und 19 zitieren
        ...
        \subsection{Raumverständnis}
            ...
            Mehrere Forschungsergebnisse deuten darauf hin, dass natürliches Gehen das Raumverständnis, im Vergleich zu anderen Fortbewegungsarten verbesset. \cite{peck-vergleich-2011, walking-improves-map-building} %TODO: mehr?

            In dieser Studie wird dieser Effekt gemessen
            %gemessen ob es auch in generierten leveln gilt?
            indem die Proband:innen nach jedem Durchlauf einer Versuchsbedingung schätzen sollen, wieviele Meter sie sich gerade fortbewegten.\footnote{Dabei ist zu spezifizieren dass es nicht um die Euklidische Entfernung von Start und Ziel ging (die \textquote{Luftlinie}), sondern um die konkret fortbewegte Strecke.}
            ...
            Um das Raumverständnis einer Proband:in zu Messen tätige ich im Kontext dieser Studie also die Annahme: \textquote{Je besser ihr Raumverständnis ist, desto besser kann sie die von ihr gegangene (oder andersartig fortbewegte) Strecke einschätzen.}
            Diese Art von Messung bietet sich für Level, die mit der hier vorgestellten Methode generiert werden an, weil sie sehr linear sind; Ein Raum folgt auf den nächsten, ohne dass die Nutzer:in Richtungsentscheidungen tätigt.

            Zwar gibt es in der Literatur viele Hinweise darauf, dass Menschen in virtuellen Umgebungen die Entfernung zwischen ihnen und Objekten in der Umgebung, verglichen mit realen Umgebungen,  nicht besonders gut einschätzen können \cite{meta-distance-perception, bruder-distance}, % TODO: \cite, RVH 13 and others TODO: all?
            %schlechter, jeh schlechter die qualität
            allerdings werden in der hier vorgestellten Studie alle drei Fortbewegungsarten in einer (sich auch nicht zwischen den Bedingungen unterscheidende) virtuellen Umgebung durchgeführt, sodass dieser Effekt ausgeglichen wird. % TODO: entscheiden ob rein nehmen oder nicht, quellen finden.

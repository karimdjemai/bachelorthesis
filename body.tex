...
Demnach wäre es also Cybersickness senkend, wenn andere Sinnesmodalitäten, wie der Gleichgewichtssinn oder die Propriozeption zusätzlich zum visuellen Sinn die eigenbewegung der Nutzer:in wahrnehmen. Und tatsächlich konnten beispielsweise Langbehn et al zeigen, dass Nutzer:innen, die sich in virtuellen Umgebungen fortbewegen signifikant weniger Cybersickness erleben, als wenn sie sich mithilfe von Joysticks fortbewegen.
% [
%     %TODO: quelle
...
Dies entsteht zum einen, durch räumlich limitierte Erfassung (auf Englisch: tracking) der Position und Rotation des Headsets und der Controller bei einigen Technologien, zum anderen durch die Raumgröße der meisten VR-Setups.
...
Eine Herangehensweise an dieses Problem sind so genannte \textquote{Redirection Techniques} (besser bekannt als Redirected Walking, diese Begriffe werden oft austauschbar verwendet, siehe \cite{redwalk_uebersicht})). Dies ist ein Sammelbegriff für Methoden bei denen die Nutzer:in durch subtile Manipulationen an der Darstellung der virtuellen Umgebung, auf virtuelle Bereiche zugreifen kann, die sonst ausserhalb des begehbaren Bereichs wären. Tatsächlich bleibt sie dabei aber nur innerhalb des realen Trackingspaces.
...
an einem bestimmten Punkt gewisse Veränderungen zwischen der Nutzer:in und ihrem virtuellen Avatar etabliert (beispielsweise gehgeschwindigkeit), sodass die Bewegung zwischen den beiden Umgebungen sich unterscheiden.
Bei manchen dieser Methoden sorgt diese Veränderung dafür, die Bewegungen der Nutzer:in in gewisser Weise zu Manipulieren, etwa dafür zu sorgen, dass sie sich immer %curve paper nochmal anschauen

Mit diesen Methoden lassen sich also einige Effekte erzielen, wie unter anderem die Nutzer:in von den äußeren Begrenzungen des Trackingspaces fern halten, und die virtuell begehbare Fläche zu vergrößern.
...
$$ gR * \alpha $$
durchgeführt wird. Folglich rotieren virtuelle Kameras mit einem Rotationgain von $gR > 1$ schneller in die Richtung der realen Kopfdrehung als der reale Kopf, während solche mit einem Rotationgain von $gR < 1$ langsamer die Richtung drehen.
...
\chapter{Durchführung und Ergebnisse}\label{}
    ...
    \section{Ablauf der Studie}
        ...
        \section{Statistische Evaluationsverfahren}
            ...
            %errechnung % geschätzt positiv, prozent negativ ?, signifikante unterschiede?

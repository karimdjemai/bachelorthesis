...
An dieser Stelle wird beschrieben wie genau die Generierung des Levels funktioniert. Zunächst werde ich beschreiben, wie er funktioniert und dann den wesentlichen Teil des Quellcodes Abbilden, um die Implementierung vorzustellen.

\subsection{Grundidee hinter dem Algorithmus}
...
Wenn ein Rotation-Gain angewandt wird verdrehen sich virtuelle und reale Realität um den Drehpunkt, der durch die Position der Nutzer:in bestimmt wird. Wenn diese Position sich in einer Ecke des (rechteckigen) Raumes und zudem genau gleich weit von beiden Wänden, die diese Ecke bilden, entfernt ist,
Position $P$, und auf die Nutzer:in dann ein Rotation-Gain angewandt wird,
...
Die beiden Areale $A$ und $B$ überlappen sich in einem gewissen Bereich, in dem die Nutzer:in nach Beendigung des Rotation-Gains auch gerade steht, aber ein Großteil von $B$ ist Teil der virtuellen Realität der zuvor nicht zugänglich war. Auf diese Weise ist also für die Nutzer:in ein neuer Bereich der Virtuellen Welt begehbar geworden.
...
Die Grundidee hinter dem hier vorgestellten Raumgenerierungsalgorithmus ist es also, dass genau die beiden Areale $A$ und $B$ von Wänden umgeben sind. Um dann also von $A$ zu $B$ zu kommen muss die Nutzer:in sich dann an der Position $P$ oft genug drehen und ein Rotation-Gain bis zu 90\textdegree\ angewandt werden. Danach kann sie mit der Illusion geradeaus gehen einen neuen Bereich zu erkunden während sie eigentlich nur wieder zurück in Richtung Mitte des Trackingspaces geht.
